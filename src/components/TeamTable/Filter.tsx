...
import { Grid, Button, Paper, Popover, Typography, FormGroup, FormControlLabel, Divider } from '@material-ui/core';
...
export function Filter() {
  ...
  const handlePersonalStatusChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setPersonalStatus({ ...personalStatus, [event.target.name]: event.target.checked });
  }
  
  const handleTravelStatusChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setTravelStatus({ ...travelStatus, [event.target.name]: event.target.checked });
  }

  const handleMISDepartmentChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setMISDepartment({ ...MISDepartment, [event.target.name]: event.target.checked });
  }

  const handleWorkLocationChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setWorkLocation({ ...workLocation, [event.target.name]: event.target.checked });
  }
    ...
    <div>
      ...
      >
        <Paper elevation={0} className={classes.paper}>
          <Grid>
            <Typography variant="h6">Personal Status</Typography>
            <FormGroup row>
              <FormControlLabel
                control={
                  <Checkbox 
          ...
          <Divider />
          <Grid>
            <Typography variant="h6">Travel Status</Typography>
            <FormGroup row>
              <FormControlLabel
                control={
                  <Checkbox 
          ...
          <Divider />
          <Grid>
            <Typography variant="h6">MIS Department</Typography>
            <FormGroup row>
              <FormControlLabel
                control={
                  <Checkbox 
          ...
          <div className={classes.buttonWrapper}>
            <Button className={classes.clearBtn} onClick={clearFilters}>Clear all filters</Button>
            <div>
              <Button className={classes.cancelBtn} onClick={handleClose}>Cancel</Button>
              <Button className={classes.applyBtn} onClick={apply}>Apply</Button>
            </div>
          </div>
        </Paper>
      </Popover>
